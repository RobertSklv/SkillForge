...
export function searchArticlesAdvanced(gridState: GridState): Promise<PaginationResponse<ArticleCardType>> {
...
export function getReportFormOptions(authToken?: string): Promise<ReportFormOptions> {
...
export function loadTagPage(name: string, authToken?: string): Promise<TagPageData> {
...
export async function requestApi(url: string, data?: FetchData): Promise<any> {
	return request(process.env.NEXT_PUBLIC_BACKEND_DOMAIN + '/Api' + url, data);
}
...
export async function request(url: string, data?: FetchData): Promise<any> {
	...
	const response = await fetch(_url, data.init);
